...
- **`syn_gain` derived from `n_kc`.** More Kenyon cells means more co-active KCs per odor, so a fixed gain put an 80-KC and a 2000-KC circuit in different bands. Default is `950 · n_kc^(−0.70)`. The constants are an empirical fit, rounded, not measured: for each `n_kc` the gain putting an untrained circuit at ~20 Hz total MBON rate was solved by bisection over several seeds, then log-log fitted. Two independent fits (different seed sets) gave exponents −0.68 and −0.70, residuals within ~7%. Pass `syn_gain` to override.
