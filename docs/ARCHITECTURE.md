...
Store, embed, retrieve, and optional resonance stay in `emotional-memory`. On-disk memories: `SQLiteStore` (`examples/demo_persist.py`). Mood sidecar: `MoodField.to_dict` / `from_dict`. Semantic vectors: `SentenceTransformerEmbedder` (`examples/demo_embedder.py`, extra `embed`). Delayed US: `examples/demo_cs_us.py` and `AffectiveLoop(td_sequential=True)`. Resonance links: `examples/demo_resonance.py` (emotional-memory default).
