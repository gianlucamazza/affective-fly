...
| `malecns_connectome.py` | Connectome loader: `load_connectome()` from JSON/Feather/Parquet; curated Aso 2014 `map_to_aso_names()`. Published counts are scaled into LIF `w_max` (`scale_published_weights_to_band`) so `learn()` cannot flatten anatomy. `n_kc` must match the file unless `allow_kc_mismatch`. Without path, random (not connectome-backed). |
...
| `host_adapter.py` | `HostFrame` v1.0 schema (JSON contract), `HostAdapter` journal replay (save/load JSONL), outcome reporting via context fields. `mood_dts_from_timestamps` for host-study replay. `sensory_frame_to_host_frame()` stores a visual fingerprint, not a regenerating seed. |
...
| `run.py` | `live_loop`: interval ticks, SQLite + journal + `measure.jsonl`. CLI `run`. Lab taus (8/4/5) and `mood_dt=1.0`. |
| `study.py` | `host_study_loop`: hypothesis 300/60/180 + wall-clock `mood_dt` (monotonic or `--replay` timestamps). CLI `study`. Does not retune defaults. |
