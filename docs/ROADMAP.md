...
- `LIFCircuit` sub-steps at `dt_sim` (1 ms), uses delta synapses, non-negative weights, and a `syn_gain` derived from `n_kc` (`950 · n_kc^(−0.70)`), rescaled by actual KC→MBON fan-in after a published connectome load. One Euler step of the caller's 50 ms both under-sampled a 20 ms membrane and capped every rate at `1/dt`.
...
**Phase 6** (empirical, scaffolding + study runner in-repo): τ / threshold / saturation questions still need a real human or production-host session. In-repo: measurement protocol ([PHASE6_MEASUREMENT.md](PHASE6_MEASUREMENT.md)), per-tick mood/gate/approach JSONL (`MeasurementLog`, enriched journal), host-study runner (`python -m affective_fly study`: hypothesis 300/60/180 and wall-clock `mood_dt`), L3 calibrator (`python -m affective_fly calibrate`) that **reads logs and does not invent fitted values**. Lab `run` stays 8/4/5 and `mood_dt=1.0`. Do not change the `approach_tendency` denominator or retune `threshold_act` / `threshold_approach` / `threshold_calm` until that study. Do not commit fitted defaults from synthetic/CLI data.
