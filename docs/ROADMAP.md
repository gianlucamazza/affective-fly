...
**Phase 6** (empirical, scaffolding in-repo): τ / threshold / saturation questions still need a real host or user study. In-repo: measurement protocol ([PHASE6_MEASUREMENT.md](PHASE6_MEASUREMENT.md)), per-tick mood/gate/approach JSONL (`MeasurementLog`, enriched journal), L3 calibrator (`python -m affective_fly calibrate`) that **reads logs and does not invent fitted values**. Hypothesis taus stay 300 / 60 / 180; lab/CLI 8 / 4 / 5 s stays a visibility choice. Do not change the `approach_tendency` denominator or retune `threshold_act` / `threshold_approach` / `threshold_calm` until that study.
